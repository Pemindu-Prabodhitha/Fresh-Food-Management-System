...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_inquiry'])) {
    ...
    if ($subject === '' || $message_txt === '') {
        echo "<script>alert('Please fill in both subject and message.'); window.location.href='sales.php';</script>";
        exit;
    }

    $inq_sql = "INSERT INTO inquiries (user_id, subject, message) VALUES (?, ?, ?)";
    if ($stmt = mysqli_prepare($con, $inq_sql)) {
        mysqli_stmt_bind_param($stmt, "iss", $sales_id, $subject, $message_txt);
        if (mysqli_stmt_execute($stmt)) {
            ...
            while ($admin_row = mysqli_fetch_assoc($admins_res)) {
                addNotification($con, $admin_row['user_id'], "New inquiry from " . $sales_name . " (Sales Buyer): \"$subject\"");
            }
            echo "<script>alert('Your message has been sent to the admin team.'); window.location.href='sales.php';</script>";
        } else {
            echo "<script>alert('Failed to send your inquiry. Please try again.'); window.location.href='sales.php';</script>";
        }
...
if ($stmt_inq = mysqli_prepare($con, $inq_query)) {
    mysqli_stmt_bind_param($stmt_inq, "i", $sales_id);
        ...
        <div id="tab-contact-admin" class="tab-content">
            <div class="feedback-box" style="margin-top:0;">
                ...
                <form action="sales.php" method="POST" class="feedback-form">
                    ...
                    <div class="form-group">
                        <label>Subject:</label>
                        <input type="text" name="subject" class="form-control" placeholder="e.g. Issue with an order" maxlength="150" required>
                    </div>
                    <div class="form-group">
                        <label>Message:</label>
